...
//                <img src="https://res.cloudinary.com/dc8ryewn6/image/upload/v1789925600/robo_njfdlq.png" alt="Digital Clone" className="w-full h-full object-contain relative z-10 drop-shadow-2xl" />
//             </div>
...
import { 
  FaReact, FaJs, FaHtml5, FaNodeJs, FaPython, FaJava, FaGitAlt, 
  FaServer, FaRobot, FaBrain, FaNetworkWired, FaDatabase, FaCloud, 
  FaCog, FaCode, FaWind, FaLayerGroup, FaBolt, FaGlobe 
} from 'react-icons/fa';
import { RiTailwindCssLine } from "react-icons/ri";
import { SiFramer , SiGsap , SiExpress , SiLangchain  } from "react-icons/si";
...
// --- COMPONENT 2: Grid Skill Box ---
const GridSkillBox = ({ id, innerGrid, items, isColor, boxClass = "h-full" }) => {
  const boxBg = isColor 
    ? 'bg-[#0a0e29] border-white/20 shadow-[6px_6px_0px_rgba(255,255,255,0.05)]' 
    : 'bg-white border-black/10 shadow-[8px_8px_0px_rgba(0,0,0,0.08)]';
    ...
    >
      ...
      <div className={`w-full grid ${innerGrid} gap-3 mt-auto`}>
        {items.map((skill, index) => {
          ...
          return (
            <div key={index} className={`flex flex-col items-center justify-center gap-2 p-3 md:p-4 rounded-2xl border ${innerBoxBg} group`}>
              <div className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center transition-transform group-hover:scale-110 duration-300 ${textColor}`}>
  ...
  const gridData = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Extraction', icon: FaCog },
        { name: 'Pipelines', icon: FaServer },
      ]
    },
    {
      id: '04 / DATA',
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Browser Auto', icon: FaCode },
        { name: 'Workflows', icon: LuWorkflow },
      ],
      bottomText: (
        <div className="hidden md:block mt-6 md:mt-8 ml-4 md:ml-6">
           <span className={`font-mono text-xs md:text-sm uppercase tracking-widest font-bold px-4 py-2 rounded-lg border ${isColor ? 'bg-white/5 border-white/10 text-[#00f0ff]' : 'bg-black text-white border-black'}`}>
             root@palak:~# <span className="opacity-75">same_curiosity // different_domains.sys</span>
           </span>
        </div>
