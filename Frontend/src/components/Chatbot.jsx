      ...
      >
        ...
        <form onSubmit={handleSendMessage} className={`p-3 md:p-5 border-t-4 flex gap-3 items-center shrink-0 ${isColor ? 'bg-black/60 border-white/20' : 'bg-[#F4F4F0] border-black'}`}>
          ...
          <motion.button 
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className={`px-3.5 py-3 md:px-5 rounded-xl font-mono text-xs font-black uppercase tracking-widest border-2 cursor-pointer transition-all flex items-center justify-center ${
              isColor 
                ? 'bg-cyan-400 text-black border-cyan-400 hover:bg-white' 
                : 'bg-[#ff00ea] text-white border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-black'
            }`}
          >
            ...
            <svg className="md:hidden w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
